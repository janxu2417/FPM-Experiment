...
if ~isfield(opts,'use_internal_intensity_correction')
    opts.use_internal_intensity_correction = true;
end
...
use_internal_intensity_correction = opts.use_internal_intensity_correction;
...
%{
figure();
subplot(121);imshow(abs(himFT(50:end-50,50:end-50)),[]);title('Amplitude');
subplot(122);imshow(angle(himFT(50:end-50,50:end-50)),[]);title('Phase');
%}
...
for i = 1:loopnum % 迭代次数
    for i3 = 1:numim
        ...
        if use_internal_intensity_correction
            mean_imseqlow = mean(mean(pratio^2*abs(imseqlow(:,:,i3))));
            if mean_imseqlow > 0.05
        ...
        if mod(i3, 10) == 0
            fprintf('迭代状态: i=%d, i3=%d, tt均值=%f, himFT均值=%f\n', ...
                i, i3, mean(tt(1:1+(i-1)*numim:i3+(i-1)*numim)), mean(abs(himFT(:))));
        end
